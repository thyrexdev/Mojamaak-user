  ...
  const fetchBuildings = async (targetPage = 1) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/dashboard/complex-admin/buildings?per_page=10&page=${targetPage}`,
    ...
    <div className="p-6 font-arabic" dir="rtl">
      <Card className="bg-white shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between p-6 pb-4">
          <div>
            <CardTitle className="text-lg font-semibold text-gray-900">إدارة المباني والشقق</CardTitle>
            <p className="text-sm text-gray-500">إضافة المباني داخل المجمع وتحديد الشقق المتوفرة فيها.</p>
          </div>
          <Button
                ...
                <TableBody>
                  {buildings.map((building) => (
                    <React.Fragment key={building.id}>
                      ...
                      >
                        <TableCell className="text-center">{building.id}</TableCell>
                        <TableCell className="text-center">{building.address}</TableCell>
                        <TableCell className="text-center">{building.description}</TableCell>
                        <TableCell className="text-center">{building.apartments.length}</TableCell>
                      ...
                      {expandedId === building.id &&
                        building.apartments.map((apt) => (
                          <TableRow key={`apt-${building.id}-${apt.id}`} className="bg-gray-100 border-t border-gray-300">
                            <TableCell colSpan={6}>
                              <div className="flex justify-between items-center">
                                <div className="text-sm text-right space-y-1">
                                  ...
                                  <div>الحالة: {apt.is_available ? "متوفرة" : "محجوزة"}</div>
                                  {apt.residency_date && (
                                    <div>
                                      تاريخ السكن: {new Date(apt.residency_date).toLocaleDateString("fr-FR")}
                                    </div>
                                  )}
                                </div>
